...
class TeacherBuffer(Dataset):
    ...
    def __init__(self, data_path: str, device: torch.device, pref_rm_path: str = None,
        ...
        # Soft-TAC dedicated pool — must be initialized BEFORE _build_pref_episodes() /
        # _build_pref_episodes_gt() so the assignment inside those methods is not
        # overwritten by later __init__ code.
        self._soft_tac_expert_episodes = []   # populated in _build_pref_episodes() / _build_pref_episodes_gt()
        self.soft_tac_student_episodes = []   # populated by add_soft_tac_student_episode()
        # RM-rescore state: Soft-TAC J values start as GT placeholders and are
        # overwritten by rescore_soft_tac_pool() once the online RM is active.
        # Soft-TAC is gated in sail.py behind _soft_tac_pool_rescored=True so GT
        # J values are never used as Soft-TAC labels.
        self._soft_tac_pool_rescored: bool = False
        self._soft_tac_rescore_count: int = 0
        if pref_rm_path:
            import os
        ...
        # GT-based pref pool (online RM path): build from FULL expert data BEFORE ring truncation.
        # Caches full numpy arrays so OnlineRMManager can use all expert episodes even after the
        # transition ring buffer is truncated to max_size.
        self._full_expert_obs_np   = None
        self._full_expert_act_np   = None
        self._full_expert_rew_np   = None
        self._full_expert_dones_np = None
        if pref_build_gt and not self.pref_episodes:
            self._full_expert_obs_np   = self.states.cpu().numpy()
            self._full_expert_act_np   = self.actions.cpu().numpy()
            self._full_expert_rew_np   = self.rewards.cpu().numpy().ravel()
            self._full_expert_dones_np = self.dones.cpu().numpy().ravel()
            self._build_pref_episodes_gt()
    ...
    def _build_pref_episodes_gt(self):
        """
        Build pref pool from GT (environment) returns, using full pre-truncation data.
        Called by __init__ when pref_build_gt=True, before ring truncation.
        Each entry: {'obs': tensor(T,obs_dim), 'acs': tensor(T,act_dim), 'J': float (GT return)}
        """
        dones_np = self.dones.cpu().numpy().ravel()
        done_idx = np.where(dones_np == 1)[0]
        if len(done_idx) == 0:
            print("[TeacherBuffer] No episode boundaries found for GT pref pool.")
            return
        rewards_np = self.rewards.cpu().numpy().ravel()
        start = 0
        for last in done_idx:
            obs_ep = self.states[start:last + 1]
            acs_ep = self.actions[start:last + 1]
            J = float(rewards_np[start:last + 1].sum())
            self.pref_episodes.append({'obs': obs_ep, 'acs': acs_ep, 'J': J})
            start = int(last) + 1
        scores = [ep['J'] for ep in self.pref_episodes]
        print(f"[TeacherBuffer] Pref pool (GT): {len(self.pref_episodes)} expert episodes "
              f"(built from full dataset before ring truncation)")
        if scores:
            print(f"[TeacherBuffer] Pref pool GT returns: "
                  f"mean={np.mean(scores):.1f} min={np.min(scores):.1f} max={np.max(scores):.1f} "
                  f"spread={np.max(scores) - np.min(scores):.1f}")

        # Mirror expert episodes into the Soft-TAC dedicated pool (permanent).
        # Use independent dicts so rescore_soft_tac_pool() updating ep['J'] here
        # does not silently mutate pref_episodes entries (and vice-versa).
        self._soft_tac_expert_episodes = [
            {'obs': ep['obs'], 'acs': ep['acs'], 'J': ep['J']}
            for ep in self.pref_episodes
        ]

        self._recompute_pref_weights()

    def add_pref_episode(self, obs_np: np.ndarray, acs_np: np.ndarray, J: float) -> None:
        """
        Add an episode with a pre-computed quality score J to the pref pool.
        Does NOT require pref_rm — used by online RM path where J is supplied externally
        (GT before RM activation, RM-derived after activation).
        Applies the same quantile-based pruning as add_episode() for TF parity.
        """
        obs_t = torch.tensor(np.asarray(obs_np, dtype=np.float32), device=self.device)
        acs_t = torch.tensor(np.asarray(acs_np, dtype=np.float32), device=self.device)
        self.pref_episodes.append({'obs': obs_t, 'acs': acs_t, 'J': float(J)})
        if (self.pref_max_teacher_trajs is not None
                and len(self.pref_episodes) > self.pref_max_teacher_trajs):
            scores_arr = np.array([ep['J'] for ep in self.pref_episodes], dtype=np.float64)
            thresh = np.quantile(scores_arr, self.pref_promote_quantile)
    ...
    def add_student_episode(self, obs_ep: np.ndarray, acs_ep: np.ndarray,
                            J: float = None) -> None:
        """
        Append a completed student episode to pref_student_episodes with J label.

        J can be supplied directly (online RM path) or computed internally (offline
        pref_rm path).  If J is None and no pref_rm is loaded, this is a no-op.

        Called by SAILAdaptiveCallback at episode end when qpref_source='student'.
        Pruning: recency-based — keep last pref_max_student_trajs entries.
        """
        if J is None:
        ...
        n = len(self.pref_student_episodes)
        ...
        if n == 0:
            print(f"[TeacherBuffer] qpref student pool: first episode added  J={J:.2f}")
    ...
    def rescore_soft_tac_pool(self, online_rm_manager) -> int:
        """
        Re-score every episode in soft_tac_pool with the current online RM.
        Updates ep['J'] in-place for both expert and student components.

        Called from sail.py whenever online_rm_manager.pending_rescore fires.
        After the first call, _soft_tac_pool_rescored=True, which unlocks the
        Soft-TAC gate in sail.py — guaranteeing that GT placeholders are never
        used as Soft-TAC labels.

        Returns number of episodes successfully rescored.
        """
        pool = self._soft_tac_expert_episodes + self.soft_tac_student_episodes
        if not pool:
            return 0

        j_before = [ep['J'] for ep in pool]
        n_rescored = 0
        ...
        for ep in pool:
            try:
                ...
                r = online_rm_manager.reward(obs_np, act_np)
                ep['J'] = float(np.sum(r))
                n_rescored += 1
            except Exception as e:
                print(f"[TeacherBuffer] WARNING: soft_tac_pool rescore failed for one episode: {e}")

        if n_rescored > 0:
            self._soft_tac_pool_rescored = True
            self._soft_tac_rescore_count += 1
            j_after = [ep['J'] for ep in pool]
            delta = float(np.mean(np.abs(np.array(j_after) - np.array(j_before))))
            print(f"[TeacherBuffer] soft_tac_pool rescore #{self._soft_tac_rescore_count}: "
                  f"{n_rescored} episodes updated, "
                  f"mean|ΔJ|={delta:.2f}, "
                  f"J_std={float(np.std(j_after)):.2f}")
